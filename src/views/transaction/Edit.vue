<template>
  <div class="div container my-5">
    <div class="row justify-content-center">
      <div class="col-8">
        <router-link :to="{ name: 'transaction.index' }" class="btn btn-primary btn-sm rounded shadow">Back
        </router-link>
...
export default {
  setup() {
    ...
    onMounted(() => {
      axios.get(`http://akunt-api.test/api/transaction/${route.params.id}`)
          .then((result) => {
    ...
    function update() {
      axios.put(`http://akunt-api.test/api/transaction/${route.params.id}`, transaction)
          .then(() => {
            router.push({
